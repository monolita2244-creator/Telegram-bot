...
def main_menu():
    ...
    builder.button(text="🏗 Сменить объект", callback_data="menu_object")
...
async def cb_status(callback: types.CallbackQuery):
    ...
    percent_done = (vsego_done / PLAN_TOTAL) * 100
    remains = PLAN_TOTAL - vsego_done
    ...
    text = (
        ...
        f"из {PLAN_TOTAL} т\n"
        f"⬜️ Осталось: <b>{remains:.2f} т ({percent_remains:.2f}%)</b>\n\n"
...
async def cb_reset_confirm(callback: types.CallbackQuery):
    ...
    save_counter(1)
    await callback.message.edit_text(
        "🔄 Данные и счётчик отчётов сброшены до нуля.\n\nВыберите действие:",
        reply_markup=main_menu()
    )
    await callback.answer()

# --- Сменить объект ---
@dp.callback_query(F.data == "menu_object")
async def cb_object(callback: types.CallbackQuery, state: FSMContext):
    current = load_object()
    await callback.message.edit_text(
        f"🏗 Текущий объект:\n<b>{current}</b>\n\n"
    ...
    await state.set_state(ObjectStates.waiting_object_name)
    await callback.answer()

@dp.message(ObjectStates.waiting_object_name)
...
async def process_workers(message: types.Message, state: FSMContext):
    ...
    await message.answer("⛏ Введите выполнение за смену (в тоннах, например: 0.225):")
